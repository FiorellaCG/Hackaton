...
const Ajustes = ({ isSidebarOpen, setSidebarOpen }) => {
    ...
    const handleDeleteAccount = () => {
        showConfirm({
            ...
            message: t('settings.delete_account_desc'),
            confirmText: "Eliminar definitivamente",
            isDanger: true,
            requiresInput: true,
            inputPlaceholder: "Escribe tu correo para confirmar",
            expectedInput: usuario.correo,
            onConfirm: async () => {
                try {
                    ...
                    showError(t('settings.error_generic'));
                }
            }
        });
    };
    ...
    return (
        <main className="flex-1 lg:ml-64 p-6 lg:p-10 transition-all">
            ...
            <footer className="mt-24 py-10 border-t border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row justify-between items-center text-slate-400 dark:text-slate-600 gap-6 transition-colors">
                ...
                <p className="text-[10px] font-black uppercase tracking-widest">{usuario.brand || 'GreenTalent'} AI Platform © 2026</p>
            </footer>
        </main>
    );
};
