...
import { obtenerPerfilDashboard, cargarRecomendacionesIA } from '../../../services/dashboardService';
...
const DashboardAspirante = () => {
    ...
    const cargarDatos = async () => {
        try {
            const data = await obtenerPerfilDashboard(usuarioLocalStorage.id);
            setPerfil(data);
        } catch (error) {
    ...
    const cargarDondeRecomendar = async () => {
        ...
        try {
            const data = await cargarRecomendacionesIA();
    ...
    const handleBecomePracticante = (e) => {
        e.preventDefault();
        showConfirm({
            title: "¿Activar perfil de Practicante?",
            message: "¿Estás seguro que deseas activar tu perfil como Practicante? Esto te permitirá ser visible para empresas que buscan talento en formación.",
            confirmText: "Activar Perfil",
            onConfirm: () => {
                // Lógica para actualizar perfil
                setIsModalOnboardingOpen(false);
                showSuccess('¡Tu perfil ahora es Practicante!');
            }
        });
    };
    ...
    if (!perfil || perfil.error || perfil.perfil_completo === false) return (
                ...
                <div className="flex flex-col gap-2 mb-8 mt-4">
                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-1">
                        ...
                        {perfil.institucion_origen && (
                            <div className="flex items-center gap-2 px-3 py-1 bg-[#b1b900]/10 text-[#7a8000] dark:text-[#d4db00] text-[10px] font-black uppercase tracking-widest rounded-xl border border-[#b1b900]/20 shadow-sm animate-pulse-slow">
                                <div className="p-1 bg-[#b1b900] rounded-lg">
                                    <Building2 size={10} className="text-white" />
                                </div>
                                <span>{perfil.institucion_origen.nombre === 'INA' ? 'RECOMENDADO POR EL INA' : `RECOMENDADO POR: ${perfil.institucion_origen.nombre.toUpperCase()}`}</span>
                            </div>
                        )}
                    </div>
                </div>

                {perfil.password_plano && (
                    <motion.div
                        initial={{ opacity: 0, x: -20 }}
                        animate={{ opacity: 1, x: 0 }}
                        className="mb-8 p-4 bg-orange-50 dark:bg-orange-900/20 border-l-4 border-orange-500 rounded-r-2xl flex items-center justify-between gap-4 shadow-sm"
                    >
                        <div className="flex items-center gap-3">
                            <div className="p-2 bg-orange-100 dark:bg-orange-800/40 rounded-xl text-orange-600 dark:text-orange-400">
                                <Shield size={20} />
                            </div>
                            <div>
                                <h4 className="text-sm font-black text-orange-800 dark:text-orange-200 uppercase tracking-tight">Seguridad: Contraseña Temporal</h4>
                                <p className="text-xs text-orange-700/70 dark:text-orange-400/60 font-medium">Estás usando una clave generada automáticamente: <span className="font-black bg-orange-200/50 dark:bg-orange-800/50 px-2 py-0.5 rounded ml-1 tracking-wider">{perfil.password_plano}</span></p>
                            </div>
                        </div>
                        <button
                            onClick={() => navigate('/ajustes')}
                            className="px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl transition-all shadow-lg shadow-orange-500/20"
                        >
                            Cambiar Clave
                        </button>
                    </motion.div>
                ...
                <div className="dashboard-grid grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        <div className="dashboard-card bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-6 rounded-3xl shadow-sm">
                            <div className="flex items-center justify-between mb-8 border-b border-slate-50 dark:border-slate-800 pb-4">
                                <h3 className="flex items-center gap-2 text-slate-800 dark:text-white font-black uppercase tracking-wider text-sm !mb-0">
                                    <Calendar size={18} className="text-orange-600" /> Mis Entrevistas
                                </h3>
                                <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest bg-slate-50 dark:bg-slate-800 px-3 py-1 rounded-full border border-slate-100 dark:border-slate-700">
                                    {perfil.entrevistas?.length || 0} Programadas
                                </span>
                            </div>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {perfil.entrevistas && perfil.entrevistas.length > 0 ? perfil.entrevistas.map(ent => (
                                    <motion.div
                                        key={ent.id}
                                        whileHover={{ y: -3 }}
                                        className="p-5 rounded-2xl border border-[var(--border-color)] hover:border-orange-200 dark:hover:border-orange-800 hover:bg-orange-50/20 dark:hover:bg-orange-900/10 transition-all group cursor-pointer shadow-sm hover:shadow-md bg-[var(--bg-card)]"
                                    >
                                        <div className="flex justify-between items-start mb-3">
                                            <strong className="text-base font-black text-slate-800 dark:text-slate-200 group-hover:text-orange-600 dark:group-hover:text-orange-400 transition-colors leading-tight uppercase tracking-tight">{ent.cargo}</strong>
                                            <span className={`badge ${ent.estado === 'confirmada' ? 'badge-green' : 'badge-blue'}`}>
                                                {ent.estado}
                                            </span>
                                        </div>
                                        <div className="flex flex-col gap-2 mb-4">
                                            <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400 font-medium">
                                                <Building2 size={14} /> {ent.empresa}
                                            </div>
                                            <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400 font-medium">
                                                <Calendar size={14} /> {new Date(ent.fecha).toLocaleDateString()} a las {ent.hora}
                                            </div>
                                        </div>
                                        {ent.meet_url && (
                                            <a
                                                ...
                                                className="w-full py-2 bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl flex items-center justify-center gap-2 hover:bg-blue-700 transition-all shadow-lg shadow-blue-900/20 no-underline"
                                            >
                                                <Video size={14} /> Unirse a la reunión
                                            </a>
                                        )}
                                    </motion.div>
                                )) : (
                                    <div className="text-center py-12 col-span-full bg-slate-50 dark:bg-slate-800/30 rounded-3xl border-2 border-dashed border-slate-100 dark:border-slate-800">
                                        <Calendar size={40} className="mx-auto text-slate-200 dark:text-slate-700 mb-4" />
                                        <p className="text-xs text-slate-400 dark:text-slate-500 font-black uppercase tracking-widest">No tienes entrevistas programadas</p>
                                    </div>
                                )}
                            </div>
                        </div>
                                ...
                                {perfil.capacitaciones_inscritas && perfil.capacitaciones_inscritas.length > 0 ? perfil.capacitaciones_inscritas.map(cap => (
                                    ...
                                    <div className="text-center py-12 col-span-full bg-slate-50 dark:bg-slate-800/30 rounded-3xl border-2 border-dashed border-slate-100 dark:border-slate-800">
                                        ...
                                        <button
                                            ...
                                            className="mt-4 px-6 py-2 bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-blue-700 transition-all shadow-lg shadow-blue-900/20"
                                        >
                        ...
                        <div className="dashboard-card bg-slate-900 dark:bg-slate-800 text-white p-6 rounded-3xl border-none shadow-2xl relative overflow-hidden group">
                            ...
                            <div className="space-y-3 relative z-10">
                                ...
                                <label className="w-full py-4 bg-white/10 text-white font-black rounded-2xl text-xs flex items-center justify-center gap-2 hover:bg-white/20 transition-all border border-white/10 cursor-pointer uppercase tracking-widest">
                                    ...
                                    <input type="file" className="hidden" accept=".pdf" onChange={(e) => showSuccess("Simulación: " + e.target.files[0]?.name)} />
                                </label>
                            </div>
                        </div>
