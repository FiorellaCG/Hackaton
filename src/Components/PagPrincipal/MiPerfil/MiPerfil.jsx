  ...
  useEffect(() => {
    const cargarPerfil = async () => {
      try {
        ...
        // Si no tiene datos relevantes, consideramos que no tiene perfil aún
        if (data && data.nombre) {
          setPerfil(data);
        } else {
          setMostrarForm(true);
        ...
        // Si falla (404 por ejemplo), mostrar formulario
        setMostrarForm(true);
      } finally {
        setCargando(false);
      }
    ...
    <div>
      ...
      <section>
        ...
        <div>
          ...
          <span>{perfil.telefono_alterno || "—"}</span>
        </div>
      ...
      {/* Datos profesionales solo si es aspirante */}
      {rol === "aspirante" && perfil.nivel_educativo && (
        <section>
          ...
          <div>
            ...
            <span>{perfil.nivel_educativo}</span>
          </div>

          <div>
            <span>Estado Laboral</span>
            <span>{perfil.estado_laboral}</span>
          </div>
